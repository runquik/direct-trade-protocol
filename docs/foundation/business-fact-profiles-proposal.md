...
2. **`product@1`** (done September 23, 2026): `spec/profiles/product/1.md`, the exact contract and fixtures under `spec/profiles/product/1/`, `nullable` added to the dialect (P4), the `product-v1` semantics in the reference host with immutable `base_unit` and `tracking` (P12) and immutable published packaging versions. Packaging is carried inline (the open question is closed that way). Not yet registered: that waits for step 4.
3. **`inventory@2`** (done September 23, 2026): `spec/profiles/inventory/2.md`, the contract and fact-stream fixtures under `spec/profiles/inventory/2/` with the outcome of every fact and the exact final ledger, the reducer in `sdk/src/profiles/inventory2.ts` (P5 statuses, P6 separate reservations, P7 high-water marks with a 64-sequence window, P8 sixteen legs, P9 no aggregation), `inventory.open` and `inventory.ledger` in the reference host with replay on snapshot import, and the v1 projection `projectInventoryV1` as the bridge. Virtual locations are `~supplier`, `~customer`, `~adjustment`, `~production`; the adjustment reason is free bounded text (both open questions closed that way).
4. **A second implementation** (done September 23, 2026): `sdk/tests/interop/product-reader.ts` and `sdk/tests/interop/inventory2-reader.ts`, coded without the SDK reducer, validator, decimal helpers or canonicalizer, pass both fixture sets and agree with the reference step by step; both kinds are registered in `spec/profiles/index.json`, and hosts admit the registered contracts with `profile.admit`. Same author, separately coded: this is consumer agreement, not independent authorship.
5. **`party@1`** (done September 23, 2026: `spec/profiles/party/1.md`, contract and fixtures, `party-v1` in the reference host with the personnel-policy rule for person parties, a second implementation in `sdk/tests/interop/party-reader.ts`, registered), then **`order@1`** and the minimal **`forecast@1`** (both done September 23, 2026: `spec/profiles/order/1.md` and `spec/profiles/forecast/1.md`, contracts and fixtures including the order transition table, `order-v1` and `forecast-v1` in the reference host, second implementations in `sdk/tests/interop/order-reader.ts` and `forecast-reader.ts`, both registered). The build order is complete; every kind in section 3 is registered with a second, separately coded implementation. Independent review, a second builder and the promotion rule's 'two independent implementations' remain open, as for every unreleased slice.
