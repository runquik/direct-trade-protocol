...
|---|---|---|
| Identity | Owner-approved resolver enrollment, separate recovery authority, transactional control history, lease drainage, exact retry acknowledgments; a separately reviewed person-command authentication adapter | Complete authenticated host, cooperative resolver transfer, service sponsorship and explicit legacy migration |
| Authority | Client mandate plus agency staff delegation, narrowing, cumulative ancestor budgets, exact-plan approvals | Verified live principals/approvals/accounting connected atomically to production command acceptance |
