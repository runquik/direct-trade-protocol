...
|---|---|---|
| Identity | Owner-approved resolver enrollment, separate recovery authority, transactional control history, lease drainage, exact retry acknowledgments; a separately reviewed person-command authentication adapter; a portable, replayable identity log and owner-signed moves between resolvers (implementer-tested, not yet independently reviewed) | Complete authenticated host, move admission in the authentication adapter's durable pins, service sponsorship and explicit legacy migration |
