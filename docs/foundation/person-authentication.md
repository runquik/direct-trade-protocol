...
Higher control checkpoints learned while acting for one company prevent older control from being reused at another company on the same host. Resolver/epoch changes and configured checkpoint advances that conflict with durable state fail closed; an explicit verified admission/migration procedure is required. This is not protection against a pinned resolver forging an entirely new authorized-looking control head. The chosen resolver trust model remains relevant.
