...
- Imported operational identity files contain the stable person ID, key, pinned resolver and resolver epoch. A wallet can export its verified control history (`POST /api/log`), and the recovery file can move the identity to another host from that log alone, without the first host (`POST /api/adopt` at the destination; `POST /api/transfer` tells a cooperative former host). Companies stay at the host where they were created. Every change point in the client library (`enroll`, `applyTransition`, `adopt`) returns the host's acknowledgment together with the fresh log, already verified against the pinned resolver, so a client never has to remember to fetch it. **A client must save that log with the recovery kit every time**: an owner whose only copy is on a host that later disappears keeps their keys and loses their identity. The reference browser client in this repository only unlocks and reads; saving the log is the responsibility of whichever client performs changes. Legacy-identity migration remains open.
