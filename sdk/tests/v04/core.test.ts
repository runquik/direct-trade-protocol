...
test("v0.4 HTTP custom profile, scoped company access, head CAS and revoked replay",async()=>{
  const store=await createDtpStore();try{
    ...
    const hidden=await client.act(outsider,"workspace.view",org,{after:0,limit:10,profile_digests:[schema],kinds:[]});assert.equal(hidden.status,404);assert.ok(!JSON.stringify(hidden).includes("Synthetic Test Co"));
    ...
    const list=await client.ok(owner,"records.list",org,{after:0,limit:1,profile_digests:[schema],kinds:[]});assert.equal(list.records.length,1);assert.ok(list.next_cursor>0);
...
test("v0.4 controller governance alone cannot read personnel, export or migrate it",async()=>{
  const store=await createDtpStore();try{
    ...
    const workspace=await client.ok(owner,"workspace.view",org,{after:0,limit:100,profile_digests:[schema],kinds:[]});assert.equal(workspace.records.length,0);assert.ok(!JSON.stringify(workspace).includes("CANARY"));
