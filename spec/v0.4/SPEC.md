...
**Member names** MUST NOT contain a quotation mark (U+0022), a reverse solidus (U+005C) or a control character (U+0000 to U+001F), at any depth, in any signed or admitted JSON. Equivalently: a member name is never written with an escape sequence, so canonical JSON never escapes a name. A receiver MUST enforce this on the received text **before** passing it to a JSON parser, and MUST refuse the whole message if any member name contains a reverse solidus; checking the parsed result is not sufficient. Member values are unrestricted. A producer SHOULD check before signing. Rationale, affected runtimes and the conformance vectors (`spec/vectors/unsafe-json.json`) are in [docs/security/json-member-names.md](../../docs/security/json-member-names.md). A host refuses such a command with `400 unsafe_member_name`.
...
Dependencies are exact previously admitted profile digests. The publisher must be allowed to read them. A profile's **kind** is `<publisher_company_id>/<name>@<major>` with the major taken from its version; the namespace `dtp` is reserved for protocol kinds registered in `spec/profiles/index.json` and cannot be a publisher. A registered protocol contract enters a host through `profile.admit {digest}` (permission `profiles.publish`): the host must carry the contract as operator configuration and the registry must list its digest; the admitted profile has publisher `dtp`, is community-visible, and is provenanced by the admitting command rather than a publication. Minor versions within a kind MUST be additive, since a subscriber that selects a kind accepts every admitted minor of it. Consumers explicitly select accepted digests for record operations; a semantic-version label alone does not imply compatibility. No automatic migration, lossy mapping, standard-field override, network schema lookup or unknown-required-semantics fallback is performed. Retaining a structural custom record is not a claim to understand its business algorithm.
...
`records.list`, `records.export` and `workspace.view` accept `{after,limit,profile_digests,kinds}`. `kinds` selects profiles by **kind**, `<namespace>/<name>@<major>`: a subscriber names what it understands, not who produces it, so a company can replace a producer without touching its subscribers. A private kind (`<publisher organization id>/<name>@<major>`) expands to every admitted, accessible profile whose publisher, name and major version match; a protocol kind (`dtp/<name>@<major>`) expands to the digests the host's copy of the kind registry ([`spec/profiles/index.json`](../profiles/index.json), format `dtp-profile-kinds-1`, operator configuration like pins) lists for it and has admitted. The effective accepted set is the union with `profile_digests` and is returned as `profile_digests` on the page. A kind that selects nothing is an explicit `unsupported_profile` error, like an unknown digest. Authorization and accepted profiles filter before pagination. `next_cursor` is the last returned sequence only when another matching page exists; `null` means no next page at that observation. Preserve the greatest processed sequence when polling later; do not reset to zero because the final cursor is null. A replayed read uses current authorization. These are data APIs, not a dashboard, notification system or subscription delivery service.
...
### Stock ledger profile (`inventory-v2`)

`inventory-v2` ([`spec/profiles/inventory/2.md`](../profiles/inventory/2.md)) is one company's stock ledger for one product, opened from that product's `product-v1` head with `inventory.open {policy_id,product_id}` and read with `inventory.ledger`. A fact names the product as `resource_id`, an observation `(source_id, sequence)`, a physical `occurred_at`, the exact ledger revision it expects, one to sixteen move legs and up to sixteen reservation operations, applied atomically or not at all. Stock lives in positions keyed by lot, location and status; every change is a move between two positions, where the virtual locations `~supplier`, `~customer`, `~adjustment` and `~production` are the counterparts for receipts, shipments, count corrections (with a reason) and transformations (with a transformation id). Quantities are in the product's base unit or in a packaging revision the product published, with an exact three-place conversion, never rounded and never converted between units. Reservations are claims on available positions, separate from the physical status; serialized products move by distinct serial. Duplicates are recognized by a per-source high-water mark with a window of sixty-four sequences; an older sequence is refused as stale rather than guessed at. The store keeps the ledger and the fact index alongside the v1 pools; snapshot import replays every fact and must reproduce the ledger exactly.
